...
export default function AdminDisciplineSearch() {
  ...
  React.useEffect(() => {
    setQuery(isDisciplinePage ? (searchParams.get("q") ?? "") : "");
  }, [searchParams, isDisciplinePage]);
  ...
  React.useEffect(() => {
    ...
    fetchAdminDisciplines()
      .then((data) => {
        if (!cancelled) setDisciplines(data);
      })
      .catch(() => {
  ...
  const matches = React.useMemo(
    () => filterAdminDisciplines(disciplines, query),
    [disciplines, query],
  );

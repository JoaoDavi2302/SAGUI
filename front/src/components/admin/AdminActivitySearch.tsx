...
import type { ModuleActivityCard } from "@/services/poo/shared/types";
import { slugify } from "@/components/layout/headerConfig";

const PREVIEW_LIMIT = 6;
const database = DatabaseProvider.getDatabase();

function filterActivities(modules: ModuleActivityCard[], query: string) {
...
export default function AdminActivitySearch() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const containerRef = React.useRef<HTMLDivElement>(null);
  const { user, effectiveRole } = useUser();
  const isActivityPage = pathname.startsWith("/avaliacoes");

  const [query, setQuery] = React.useState("");
  const [modules, setModules] = React.useState<ModuleActivityCard[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [open, setOpen] = React.useState(false);
  const [activeIndex, setActiveIndex] = React.useState(-1);

  React.useEffect(() => {
    setQuery(isActivityPage ? (searchParams.get("q") ?? "") : "");
  }, [searchParams, isActivityPage]);
  ...
  React.useEffect(() => {
    ...
    const provider = ActivityProvider.create(effectiveRole, database, user);
    setModules(provider.listModules());
    setLoading(false);
  }, [user, effectiveRole]);

  const matches = React.useMemo(
    () => filterActivities(modules, query),
    [modules, query],
  );
  ...
  function goToActivity(module: ModuleActivityCard) {
    closeDropdown();
    router.push(
  ...
  function goToSearchResults() {
    ...
    router.push(
      trimmed
        ? `/avaliacoes?q=${encodeURIComponent(trimmed)}`
        : "/avaliacoes",
    );
  }
    ...
    <Box
      ...
      onSubmit={(event) => {
        ...
        if (activeIndex >= 0 && preview[activeIndex]) {
          goToActivity(preview[activeIndex]);
          return;
        }
        goToSearchResults();
      }}
                ...
                >
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    ...
                    <Typography variant="body2" color="text.secondary" noWrap>
                      {module.disciplineName} · {module.courseName}
                    </Typography>
                  </Box>
                </ListItemButton>
              ))}

...
export default function CourseDetailPage() {
  ...
  const enrolled =
    isStudent && database.enrollments?.some(
      (e: any) => e.student_id === user?.id && e.course_id === course.id
    );
          ...
          <CardContent>
            {enrolled ? (
              <>
                ...
                <LinearProgress sx={{ mt: 2, height: 8, borderRadius: 5 }} value={60} variant="determinate" />
              </>
            ) : (
              <Stack direction="row" spacing={1}  sx={{alignItems:"center"}}>
                <Lock color="disabled" />

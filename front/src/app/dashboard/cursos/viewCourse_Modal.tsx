...
export default function UserViewModal({ open, courseId, onClose }: Props) {
  const [tab, setTab] = useState(0);

  const data = useMemo(() => {
    if (!courseId) return null;

    const user = database.usuarios.find((u) => u.id === courseId);

    if (!user) return null;

    const enrollments = database.matriculas.filter(
      (m) => m.aluno_id === courseId,
    );
